...
for (i in 2:length(tabla)){
  ...
  data_na <- na.omit(a) 
  mean <- aggregate(data_na[2], FUN = "mean", by = list(format(data_na$date, "%Y-%m-%d")), na.rm= TRUE, na.action= NULL)
  upper <- aggregate(data_na[2], FUN = "max", by = list(format(data_na$date, "%Y-%m-%d")), na.rm= TRUE)
  lower <- aggregate(data_na[2], FUN = "min", by = list(format(data_na$date, "%Y-%m-%d")), na.rm= TRUE)
  sd <-  aggregate(data_na[2], FUN = "sd", by = list(format(data_na$date, "%Y-%m-%d")), na.rm= TRUE)
